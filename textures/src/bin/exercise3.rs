...
fn main() {
    ...
    // Load vertices
    let shape = vec![
        Vertex { position: [0.5, 0.5, 0.0], color: [1.0, 0.0, 0.0], texCoord: [0.55, 0.55] },
        Vertex { position: [0.5, -0.5, 0.0], color: [0.0, 1.0, 0.0], texCoord: [0.55, 0.45] },
        Vertex { position: [-0.5, -0.5, 0.0], color: [0.0, 0.0, 1.0], texCoord: [0.45, 0.45] },
        Vertex { position: [-0.5, 0.5, 0.0], color: [1.0, 1.0, 0.0], texCoord: [0.45, 0.55] },
    ];
    let indices = [
        0u8, 1, 3,
        1, 2, 3,
    ...
    // Load shaders
    let vertex_shader_src = include_str!("shaders/exercise3.vs");
    let fragment_shader_src = include_str!("shaders/exercise3.fs");
    let program = glium::Program::from_source(&window, vertex_shader_src, fragment_shader_src, None).unwrap();

    // Uniforms
    let uniforms = uniform! {
        ourTexture1: texture1.sampled().magnify_filter(glium::uniforms::MagnifySamplerFilter::Nearest),
        ourTexture2: texture2.sampled().magnify_filter(glium::uniforms::MagnifySamplerFilter::Nearest),
    };
