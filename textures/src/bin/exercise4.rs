...
fn main() {
    ...
    // Load shaders
    let vertex_shader_src = include_str!("shaders/exercise4.vs");
    let fragment_shader_src = include_str!("shaders/exercise4.fs");
    let program = glium::Program::from_source(&window, vertex_shader_src, fragment_shader_src, None).unwrap();

    let mut mix_ratio = 0.2f32;
        ...
        for event in window.poll_events() {
            match event {
                ...
                Event::KeyboardInput(ElementState::Pressed, _, Some(VirtualKeyCode::Up)) => {
                    mix_ratio += 0.1;
                    if mix_ratio > 1.0 { mix_ratio = 1.0; }
                    println!("Mix ratio: {}", mix_ratio);
                },
                Event::KeyboardInput(ElementState::Pressed, _, Some(VirtualKeyCode::Down)) => {
                    mix_ratio -= 0.1;
                    if mix_ratio < 0.0 { mix_ratio = 0.0; }
                    println!("Mix ratio: {}", mix_ratio);
                },
                _ => {},
            }
