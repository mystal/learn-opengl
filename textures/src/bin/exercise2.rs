...
fn main() {
    ...
    // Load vertices
    let shape = vec![
        Vertex { position: [0.5, 0.5, 0.0], color: [1.0, 0.0, 0.0], texCoord: [2.0, 2.0] },
        Vertex { position: [0.5, -0.5, 0.0], color: [0.0, 1.0, 0.0], texCoord: [2.0, 0.0] },
        Vertex { position: [-0.5, -0.5, 0.0], color: [0.0, 0.0, 1.0], texCoord: [0.0, 0.0] },
        Vertex { position: [-0.5, 0.5, 0.0], color: [1.0, 1.0, 0.0], texCoord: [0.0, 2.0] },
    ];
    let indices = [
        0u8, 1, 3,
        1, 2, 3,
    ...
    let uniforms = uniform! {
        ourTexture1: texture1.sampled().wrap_function(glium::uniforms::SamplerWrapFunction::Clamp),
        ourTexture2: texture2.sampled().wrap_function(glium::uniforms::SamplerWrapFunction::Repeat),
    };
