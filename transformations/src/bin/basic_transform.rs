...
fn main() {
    ...
    // Load shaders
    let vertex_shader_src = include_str!("shaders/basic_transform.vs");
    let fragment_shader_src = include_str!("shaders/basic_transform.fs");
    let program = glium::Program::from_source(&window, vertex_shader_src, fragment_shader_src, None).unwrap();

    // Create the transformation matrix
    let rotation_axis = cgmath::vec3(0.0f32, 0.0, 1.0);
    let rotation_angle = cgmath::deg(90.0);
    let rotation: cgmath::Matrix4<f32> = cgmath::Matrix3::from_axis_angle(rotation_axis, rotation_angle.into()).into();
    let scale = cgmath::Matrix4::from_scale(0.5f32);
